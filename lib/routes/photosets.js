...
router.get('/list', async (ctx, next) => {
    ...
    const simplifiedPhotosets = _.map(gotPhotosets, (photoset) => _.assign({ url_m: _.get(photoset, 'primary_photo_extras.url_m') }, _.pick(photoset, ['id', 'photos', 'title'])));
    ctx.body = {
...
router.post('/reorder', async (ctx, next) => {
    ...
    ctx.assert(_.includes(['datetaken', 'dateupload', 'title', 'views'], ctx.request.mergedBody.orderBy), 400, '"orderBy" must be one of "datetaken", "dateupload", "title", "views"');
